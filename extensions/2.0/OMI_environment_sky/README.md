...
The `"OMI_environment_sky"` extension is defined at the root of the glTF document, in the `"extensions"` object. It holds an array of sky resources, and an integer that references a sky to use as the default sky.

|           | Type       | Description                                                             | Default value |
| --------- | ---------- | ----------------------------------------------------------------------- | ------------- |
| **skies** | `object[]` | The skies available to use in the scene.                                | `[]`          |
| **sky**   | `integer`  | The index of the sky to use by default, if not overridden by the scene. | 0             |

Only one sky may be active at a time. In most situations, glTF files will only contain one sky. However, a glTF file may contain multiple skies if it contains multiple scenes, or includes a mechanism for changing the sky at runtime, such as by animation or script.
...
|         | Type      | Description                                             | Default value |
| ------- | --------- | ------------------------------------------------------- | ------------- |
| **sky** | `integer` | The index of the sky to use for this scene, if defined. | -1            |

The scene-level sky index is an override for the document-level sky index. If the scene does not have a `"sky"` set, use the value from the document-level extension.
